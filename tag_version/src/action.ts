...
import { gte, inc, parse, ReleaseType, valid } from 'semver'
...
const fillTemplate = (templateString: string, templateVars: object): string => {
...
export default async function main(): Promise<void> {
    ...
    const customFormat = core.getInput('custom_format')
    const useCustomFormatFor = core.getInput('custom_format_for')
    const skipChangeLog = core.getInput('skip_changelog')
    ...
    if (latestTag === undefined) {
        throw new Error("cannot get latest tag")
    }
    ...
    if (customTag) {
        ...
        if (customFormat) {
            switch (useCustomFormatFor.toLowerCase()) {
                case 'custom_branch':
                    useCustomFormat = !isPreReleaseBranch && !isReleaseBranch && !isPullRequest
                    break
                case 'custom_branch_and_pr':
                    useCustomFormat = !isPreReleaseBranch && !isReleaseBranch
                    break
                case 'prerelease':
                    useCustomFormat = isPrerelease
                    break
                case 'all':
                    useCustomFormat = true
                    break
                case 'release':
                    useCustomFormat = isReleaseBranch && !isPrerelease && !isPullRequest
                    break
                case 'pr':
                    useCustomFormat = isPullRequest
                    break
                default:
                    useCustomFormat = useCustomFormatFor === releaseType
                    break
            }
        }

        if (useCustomFormat && customFormat !== '') {
            const customVersion = fillTemplate(
                customFormat,
                {
                    bump,
                    previousVersion,
                    previousTag,
                    commitRef,
                    commitRefShort: commitRef.substring(0, 6),
                    nextVersion: parse(incrementedVersion),
                    currentBranch
                }
            )
            if (!valid(customVersion)) {
                core.setFailed(`${customVersion} is not a valid semver.`)
                return
            }
    ...
    if (skipChangeLog && skipChangeLog.toLocaleLowerCase() !== 'true') {
