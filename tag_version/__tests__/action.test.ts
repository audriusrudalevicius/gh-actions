...
import { expect, it, beforeAll, jest, describe, beforeEach } from "@jest/globals"
import {
    loadDefaultInputs,
    setBranch,
    setCommitSha,
...
describe('github-tag-action', () => {
    ...
    describe('pre-release branches', () => {
        beforeEach(() => {
            ...
            setInput('custom_format', '')
            setInput('custom_format_for', '')
        })
        ...
        it('does create tag with custom formatting', async () => {
            /*
             * Given
             */
            setInput('custom_format_for', 'prerelease')
            setInput('custom_format', '${this.previousVersion.major}.${this.previousVersion.minor}.${this.previousVersion.patch}-${this.currentBranch}+${this.commitRefShort}')
            const commits = [
                {
                    message: 'feat: some new feature on a pre-release branch',
                    hash: null,
                },
                { message: 'fix: this should make a preminor', hash: null },
            ]
            jest
                .spyOn(utils, 'getCommits')
                .mockImplementation(async () => commits)
    ...
    describe('other branches', () => {
        ...
        it('does create tag with custom formatting', async () => {
             ...
             * Given
             */
            setInput('custom_format_for', 'custom_branch')
            setInput('custom_format', '${this.previousVersion.major}.${this.previousVersion.minor}.${this.previousVersion.patch}-${this.currentBranch}+${this.commitRefShort}')
            const commits = [
                {
                    message:
             ...
             * Then
             */
            expect(mockSetOutput).toHaveBeenCalledWith('new_version', '1.2.3-development+79e0ea')
            expect(mockCreateTag).not.toBeCalled()
            expect(mockSetFailed).not.toBeCalled()
        })
    })
